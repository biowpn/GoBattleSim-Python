...
def set_default_game_master(game_master):
    if isinstance(game_master, str):
        ...
        raise TypeError("For game_master: Expected str or dict or GameMaster, got {}".format(
            type(game_master)))
...
class RequestHandler:
    ...
    def handle_request(self, req):
        ...
        if "GameMasterSupplement" in req["reqInput"]:
            GameMaster.CurrentInstance.from_json_join(
                req["reqInput"].pop("GameMasterSupplement"))
        elif "GameMaster" in req["reqInput"]:
            GameMaster.CurrentInstance.from_json(
                req["reqInput"].pop("GameMaster"))

        method_name = "handle_request_" + req["reqType"]
        if hasattr(self, method_name):
            print("ready to handle request via {}".format(method_name))
